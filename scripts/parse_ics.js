...
function parseICSTime(value, tzid){
...
(async ()=>{
  try{
    ...
    for(const v of vevents){
      ...
      const getTz=(l)=>{const m=l.match(/TZID=([^:;]+)/); return m?m[1]:null};
      const dtStartVal = getVal(dtstartLine);
      const dtEndVal = getVal(dtendLine);
      let start = dtStartVal?parseICSTime(dtStartVal, getTz(dtstartLine)):null;
      let end = dtEndVal?parseICSTime(dtEndVal, getTz(dtendLine)):null;
    ...
    for(const day of days){
      ...
      for(const ev of events){
        if(ev.rrule){ /* skipping recurring expansion for now */ }
    ...
    console.log(JSON.stringify({unavailableDates, slotsByDate},null,2));
